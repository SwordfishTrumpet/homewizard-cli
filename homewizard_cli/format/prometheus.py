...
def write_prometheus(data: DataResponse, console: Console):
    ...
    if data.active_power_l2_w is not None:
        lines.extend(
            [
                "",
                "# HELP p1_active_power_l2_w Phase 2 active power in watts",
                "# TYPE p1_active_power_l2_w gauge",
                f"p1_active_power_l2_w {data.active_power_l2_w}",
            ]
        )
    if data.active_power_l3_w is not None:
        lines.extend(
            [
                "",
                "# HELP p1_active_power_l3_w Phase 3 active power in watts",
                "# TYPE p1_active_power_l3_w gauge",
                f"p1_active_power_l3_w {data.active_power_l3_w}",
            ]
        )
    ...
    if data.active_voltage_l2_v is not None:
        lines.extend(
            [
                "",
                "# HELP p1_active_voltage_l2_v Phase 2 voltage",
                "# TYPE p1_active_voltage_l2_v gauge",
                f"p1_active_voltage_l2_v {data.active_voltage_l2_v}",
            ]
        )
    if data.active_voltage_l3_v is not None:
        lines.extend(
            [
                "",
                "# HELP p1_active_voltage_l3_v Phase 3 voltage",
                "# TYPE p1_active_voltage_l3_v gauge",
                f"p1_active_voltage_l3_v {data.active_voltage_l3_v}",
            ]
        )
    if data.active_frequency_hz is not None:
        lines.extend(
            [
                "",
                "# HELP p1_active_frequency_hz Line frequency in Hz",
                "# TYPE p1_active_frequency_hz gauge",
                f"p1_active_frequency_hz {data.active_frequency_hz}",
            ]
        )
