...
def _dumps_json(data: Any, indent: bool = False, default: Callable | None = str) -> str:
    ...
    if _HAS_ORJSON:
        ...
        return _orjson.dumps(data, option=opts, default=default).decode("utf-8")
    return json.dumps(data, indent=2 if indent else None, default=default)


def _loads_json(data: str | bytes) -> Any:
    """Fast JSON deserialization using orjson with stdlib fallback."""
    if _HAS_ORJSON:
...
def _crc16(data: bytes) -> int:
    """Compute CRC16-IBM (CRC-16/ARC) checksum using lookup table."""
    crc = 0x0000
    for byte in data:
