...
def test_write_data_dispatches_to_correct_formatter():
    ...
    assert "wifi_ssid" in _buf.getvalue()

    c, _buf = _console()
    write_data(data, Format.CSV, c)
    assert "active_power_w" in _buf.getvalue()

    c, _buf = _console()
    write_data(data, Format.PROMETHEUS, c)
    assert "p1_active_power_w" in _buf.getvalue()

    c, _buf = _console()
    write_data(data, Format.INFLUX, c)
    assert "p1_meter" in _buf.getvalue()

    c, _buf = _console()
    write_data(data, Format.ENV, c)
    assert "export P1_" in _buf.getvalue()

    c, _buf = _console()
    write_data(data, Format.MINIMAL, c)
    assert "500.0 W" in _buf.getvalue()

    c, _buf = _console()
    write_data(data, Format.RAW, c)
    assert "/ISKRA" in _buf.getvalue()

    c, _buf = _console()
    write_data(data, Format.TSV, c)
    assert "\t" in _buf.getvalue()
